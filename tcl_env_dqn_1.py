...
class MicroGridEnv(gym.Env):
    ...
    def render(self,name=''):
        ...
        if self.time_step==self.iterations:
            ...
            ax.yaxis.grid(True)
            plt.plot(np.array(TEMP_RENDER), color='k')
            plt.title("Outdoors Temperatures")
            plt.xlabel("Time (h)")
            plt.show()
            ...
            plt.bar(x=np.array(np.arange(self.iterations))-0.2,height=TCL_CONTROL_RENDER,width=0.2)
            plt.bar(x=np.array(np.arange(self.iterations)),height=TCL_CONSUMPTION_RENDER,width=0.2)
            plt.title("TCL_CONTROL VS TCL_CONSUMPTION")
            plt.legend(['Energy allocated for TCLs','Energy consumed by TCLs'])
            plt.xlabel("Time (h)")
            plt.ylabel("kW")
            plt.show()
